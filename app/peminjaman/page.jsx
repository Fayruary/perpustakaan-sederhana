...
export default function PeminjamanPage() {
  ...
  const handleBayar = async (denda) => {
    try {
      const res = await fetch("/api/qris", {
        ...
        body: JSON.stringify({
          id_denda: denda.id_pinjam, // atau id_denda kalau ada
      ...
      if (data?.data?.qr_url) {
        ...
        setShowQR(true); // buka popup QRIS
      } else {
        Swal.fire("Gagal", "Gagal membuat QRIS", "error");
      }
    } catch (err) {
      console.error(err);
      Swal.fire("Error", "Terjadi kesalahan saat membuat QRIS", "error");
    }
  };
  ...
  const dendaList =
    role === "siswa"
      ? data.filter((p) => p.status === "Dipinjam" && p.terlambat_hari > 0)
      : [];
      ...
      <main className="flex-1 md:ml-64 p-10 pt-10">
            ...
            <div className="mt-12 bg-white/80 backdrop-blur-sm rounded-xl shadow-lg p-6">
              ...
              {dendaList.length === 0 ? (
                <p className="text-center text-gray-500">
                  Belum ada denda untuk akun ini.
                </p>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full border border-gray-200 min-w-[700px]">
                    ...
                    <tbody>
                      {dendaList.map((d) => (
                        <tr
                          key={d.id_pinjam}
                          className="text-center border-t text-gray-700 hover:bg-white/40"
                        >
                          ...
                          <td className="p-3">
                            {new Date(d.tanggal_pinjam).toLocaleDateString(
                              "id-ID"
                            )}
                          </td>
                          <td className="p-3">
                            {new Date(d.tanggal_kembali).toLocaleDateString(
                              "id-ID"
                            )}
                          </td>
                          <td className="p-3 font-semibold">
                            {d.terlambat_hari} hari
                          </td>
        ...
        {showQR && (
          <div className="fixed inset-0 flex items-center justify-center z-[9999] pointer-events-none">
            <div className="bg-white p-5 rounded-xl shadow-2xl w-72 text-center pointer-events-auto border">
              ...
              {qrURL ? (
                <img
                  src={qrURL}
                  alt="QRIS"
                  className="w-56 h-56 mx-auto mb-3 border rounded-lg"
                />
              ) : (
                <p>Memuat QR...</p>
              )}
